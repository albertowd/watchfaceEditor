...
const warning = () => {
  ...
  if (window.last_time) {
    if (time - 1000 * 30 - window.last_time < 0) return;
  }
  window.last_time = time;
  UIkit.notification(
    'watchfaceEditor for Amazfit GTS is in development. Be careful if you use it!',
    {
      status: 'danger',
      pos: 'top-left',
      timeout: 7500
...
let wf_data = {
  import: function (json) {
    ...
    if ('Date' in json) {
      if ('WeekDay' in json.Date) data.dateWeekday = json.Date.WeekDay;
      if ('MonthAndDay' in json.Date) {
        data.monthandday = {
          TwoDigitsMonth:
            'TwoDigitsMonth' in json.Date.MonthAndDay
              ? json.Date.MonthAndDay.TwoDigitsMonth
              : true,
          TwoDigitsDay:
            'TwoDigitsDay' in json.Date.MonthAndDay
              ? json.Date.MonthAndDay.TwoDigitsDay
              : true
        };
    ...
    if ('Battery' in json) {
      if ('Images' in json.Battery) data.batteryIcon = json.Battery.Images;
      if ('Text' in json.Battery) data.batteryText = json.Battery.Text;
      if ('Scale' in json.Battery) data.batteryScale = json.Battery.Scale;
    }
    if ('Status' in json) {
      if ('Alarm' in json.Status) data.statAlarm = json.Status.Alarm;
      ...
      if ('Lock' in json.Status) data.statLock = json.Status.Lock;
    }
    if ('Activity' in json) {
      if ('Calories' in json.Activity)
        data.actCalories = {
          Number: json.Activity.Calories
        };
      if ('Steps' in json.Activity)
        data.actSteps = {
          Number: json.Activity.Steps.Step
        };
      if ('StepsGoal' in json.Activity)
        data.actStepsGoal = {
          Number: json.Activity.StepsGoal
        };
      if ('Pulse' in json.Activity)
        data.actPulse = {
          Number: json.Activity.Pulse
        };
  ...
  export: function (data) {
    ...
    if ('time' in obj) {
      ...
      if ('seconds' in obj) packed.Time.Seconds = obj.seconds;
      if ('amPm' in obj) packed.Time.AmPm = obj.amPm;
    }
    if (obj.dateWeekday || obj.monthandday) {
      packed.Date = {};
      if ('dateWeekday' in obj) packed.Date.WeekDay = obj.dateWeekday;
    ...
    if (obj.statAlarm || obj.statBluetooth || obj.statDnd || obj.statLock) {
      packed.Status = {};
      if ('statAlarm' in obj) packed.Status.Alarm = obj.statAlarm;
      if ('statBluetooth' in obj) packed.Status.Bluetooth = obj.statBluetooth;
      if ('statDnd' in obj) packed.Status.DoNotDisturb = obj.statDnd;
      if ('statLock' in obj) packed.Status.Lock = obj.statLock;
    }
    if (obj.batteryIcon || obj.batteryText || obj.batteryScale) {
      packed.Battery = {};
      if ('batteryIcon' in obj) packed.Battery.Images = obj.batteryIcon;
      if ('batteryText' in obj) packed.Battery.Text = obj.batteryText;
      if ('batteryScale' in obj) packed.Battery.Scale = obj.batteryScale;
    }
    if (
      obj.actCalories ||
      obj.actSteps ||
      ...
      if ('actSteps' in obj)
        packed.Activity.Steps = {
          Step: obj.actSteps.Number
        };
      if ('actStepsGoal' in obj)
        packed.Activity.StepsGoal = obj.actStepsGoal.Number;
      if ('actPulse' in obj) packed.Activity.Pulse = obj.actPulse.Number;
      if ('actDistance' in obj) packed.Activity.Distance = obj.actDistance;
      if ('stepsGoal' in obj) packed.Activity.StarImage = obj.stepsGoal;
    }
    if (obj.stepscircle || obj.stepsLinear || obj.stepsGoal) {
      packed.StepsProgress = {};
      if ('stepscircle' in obj) packed.StepsProgress.Circle = obj.stepscircle;
      if ('stepsLinear' in obj) packed.StepsProgress.Linear = obj.stepsLinear;
      if ('stepsGoal' in obj) packed.StepsProgress.GoalImage = obj.stepsGoal;
    }
    if (
      obj.analoghours_image ||
      obj.analogminutes_image ||
      obj.analogseconds_image
    ) {
